...
function ht_decode($url)
{
	$ht_code2 = array('k*0','k*1','k*2','k*3','k*4','k*5','k*6','k*7','k*8','k*9');
    if(preg_match('#picasaweb.google.com/kai0205/(.*?)#s', $url))
    {
		$ht_code1 = array('=','t','h','a','n','h','p','h','u','c');
        $url = explode('phim60s/',$url);
        $url=base64_decode(base64_decode(str_replace($ht_code2,$ht_code1,$url[1])));
    }
	return $url;
}
...
if($fileout!=""){
	...
	$text = get_curl($link);
	if(preg_match('#picasaweb.google.com/(.*?)#s', $link)){
		$text = explode('"media":{',$text);
		$text = explode(',"ccOverride"',$text[1]);
		$text = $text[0];
	}
	echo $text;
}
