...
/*	        if ($page && $page > 1) {
	...
	function rewrite_title($header) {
		...
		$phim_en=get_post_meta($post->ID, "phim_en", true);
		$tap=" tập ".get_name(get_query_var('ep'))." ";
		$post = $wp_query->get_queried_object();
		...
		if (is_home() && !$this->is_static_posts_page()) {
            ...
            $new_title = str_replace('%blog_title%', $this->internationalize(get_bloginfo('name')), $title_format);
            $new_title = str_replace('%blog_description%', $this->internationalize(get_bloginfo('description')), $new_title);
            if(get_query_var('ep')!="" || get_query_var('sv')!=""){
				$new_title = str_replace('%post_title%', "Xem phim ".get_the_title().$tap." | ".$phim_en.$tap, $new_title);
			}elseif(get_post_type( $post->ID )=="post") {
				$new_title = str_replace('%post_title%', 'Phim '.$title, $new_title);
			}else {$new_title = str_replace('%post_title%', $title,$new_title);}
